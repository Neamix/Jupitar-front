<template>
  <div class="modal bg-white rounded-md min-w-992 max-w-lg grid grid-cols-1">
      <div class="p-5 relative">
          <h2 class="font-extrabold text-headline ">Change your avatar</h2>
...
export default {
  ...
  methods: {
    ...mapActions(useAuthStore,['modifyAvatar']),
    uploadingFile() {
      this.uploading = true;
      this.modifyAvatar(this.images.avatar).then((response) => {
    ...
    onAddFile(error, fileObject) {
      if ( ! error ) {
        this.images.avatar = fileObject.file;
      }
    },
